...
# Dendrogram, customize the plot to remove labels
...
# Integrate Clust_Data with the groups formed by HC_Model1
...
# Load final groups data with Cluster column in RFM_data
...
# Check for the length of unique StockCode and Description
length(unique(rules_data$StockCode))
length(unique(rules_data$Description))

# Check the format of Description by viewing few rows data
...
# Correcting the type errors in Description (ProductName)
...
# structure of Association data
str(Association_data)

# Remove InvoiceNo column
Association_data$InvoiceNo <- NULL

# Rename the column name of association data as items
colnames(Association_data) <- c("items")

# Change the type of items to factor
...
# Read the csv file data as transactions
library(arules)
